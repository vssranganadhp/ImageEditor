(function( $ ){
  ...
  var init = function(index, input, options) {
    ...
    var $reset, $complete, $canvas, ctx, image;
    var draw, mousedown, stopdrag, move, resize, reset, complete, rightclick, record;
    ...
    $(input).data("points",points);
    $reset = $('<button type="button" class="btn"><i class="icon-trash"></i>Clear</button>');
    $complete = $('<button type="button" class="btn">Get Coords</button>');
    $canvas = $('<canvas>');
    ctx = $canvas[0].getContext('2d');

    image = new Image();
    resize = function() {
      $canvas.attr('height', image.height/devicePixelRatio).attr('width', image.width/devicePixelRatio);
      $canvas.css("background-size",image.width/devicePixelRatio+"px "+image.height/devicePixelRatio+"px");
      draw();
    };
    $(image).load(resize);
    ...
    complete = function(){
      $("#image_editor_points").val(points.toString());
      $("#image_editor_coords_div").show();
    }
	...
	$(document).ready( function() {
		$(input).after('', $canvas, '<br>', $reset, $complete);
		$reset.click(reset);
